...
export default async function ReportesPage() {
  ...
  const chartData = Object.entries(revenueByDay)
    .map(([date, total]) => ({
      date: new Date(date + "T12:00:00").toLocaleDateString("es-CR", { day: "numeric", month: "short" }),
      total,
    }))
    .sort((a, b) => a.date.localeCompare(b.date));

  const monthName = now.toLocaleDateString("es-CR", { month: "long", year: "numeric" });
    ...
    <div className="min-h-screen bg-[#fafafa] font-sans text-slate-900">
      ...
      <div className="lg:pl-60">
        <main className="mx-auto max-w-4xl px-4 py-8 lg:px-8 lg:py-10">
          <div className="mb-8">
            <h1 className="serif-heading text-2xl font-semibold tracking-tight text-slate-900">Reportes</h1>
            <p className="mt-1 text-sm text-slate-500 capitalize">{monthName}</p>
          </div>

          {/* Cards */}
          <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-xl border border-slate-100 bg-white p-5">
              <p className="text-xs font-medium uppercase tracking-wider text-slate-400">Ingresos del mes</p>
              <p className="mt-2 text-3xl font-semibold text-[#e9cece]">₡{totalRevenue.toLocaleString()}</p>
            </div>
            <div className="rounded-xl border border-slate-100 bg-white p-5">
              <p className="text-xs font-medium uppercase tracking-wider text-slate-400">Citas completadas</p>
              <p className="mt-2 text-3xl font-semibold text-green-600">{totalCompleted}</p>
            </div>
            <div className="rounded-xl border border-slate-100 bg-white p-5">
              <p className="text-xs font-medium uppercase tracking-wider text-slate-400">Citas canceladas</p>
              <p className="mt-2 text-3xl font-semibold text-red-400">{totalCancelled}</p>
            </div>
          </div>

          {/* Gráfico */}
          <div className="rounded-xl border border-slate-100 bg-white p-6">
            <h2 className="mb-6 text-sm font-semibold text-slate-900">Ingresos por día</h2>
            {chartData.length === 0 ? (
              <div className="flex h-48 items-center justify-center">
                <p className="text-sm text-slate-400">No hay ingresos registrados este mes.</p>
              </div>
            ) : (
