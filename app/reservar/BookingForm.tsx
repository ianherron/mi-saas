...
export default function BookingForm({
  services, timeSlots, extras, businessId, createAppointment,
}: {
  services: Service[];
  timeSlots: TimeSlot[];
  extras: Extra[];
    ...
    return (
      <div className="flex w-full max-w-[800px] flex-1 flex-col items-center justify-center gap-6 px-4 py-20 text-center">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Tu reserva fue registrada exitosamente.</p>
        </div>
        <div className="w-full max-w-sm rounded-xl border border-[#e9cece]/20 bg-white p-5 text-left">
          <div className="divide-y divide-slate-50">
            ...
            <div className="flex justify-between py-2.5">
              ...
              <span className="text-sm font-medium text-slate-900">{totalDuration} min</span>
            </div>
            <div className="flex justify-between py-2.5">
            ...
            <div className="flex justify-between py-2.5">
              ...
              <span className="text-sm font-semibold text-[#e9cece]">₡{selectedService?.price.toLocaleString()}</span>
            </div>
          </div>
        </div>
        <button
          type="button"
          onClick={() => { setConfirmed(false); setSelectedExtras([]); setDate(""); setTime(timeSlots[0]?.time ?? ""); }}
  ...
  return (
    <form action={handleSubmit} className="flex w-full max-w-[800px] flex-col gap-10">
      <input type="hidden" name="duration" value={totalDuration} />
      <input type="hidden" name="business_id" value={businessId} />

      <div className="flex flex-col gap-2 px-4">
        <h2 className="text-4xl font-black tracking-tight text-slate-900">Reserva tu cita</h2>
        <p className="text-slate-500">Disfruta una experiencia premium de cuidado de uñas adaptada a tu estilo.</p>
      </div>

      {/* Paso 1 — Servicio */}
      <section className="flex flex-col gap-4">
        <div className="flex items-center gap-2 px-4">
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e9cece] text-sm font-bold text-slate-900">1</span>
            ...
            return (
              <label
                ...
                className={`group relative flex cursor-pointer flex-col gap-3 rounded-xl border-2 bg-white p-4 shadow-sm transition-all hover:shadow-md ${
                  isSelected ? "border-[#e9cece]" : "border-transparent hover:border-[#e9cece]/50"
                }`}
              >
                ...
                <div className="flex items-end justify-between">
                  ...
                  <p className="text-xl font-bold text-[#e9cece]">₡{service.price.toLocaleString()}</p>
                </div>
              </label>
            );
          })}
      ...
      {extras.length > 0 && (
        <section className="flex flex-col gap-4 px-4">
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e9cece]/30 text-sm font-bold text-slate-900">2</span>
              ...
              return (
                <button key={extra.id} type="button" onClick={() => toggleExtra(String(extra.id))}
                  className={`flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-medium transition-colors ${
                    isActive ? "border-[#e9cece] bg-[#e9cece]/20 text-slate-900" : "border-slate-200 text-slate-600 hover:border-[#e9cece]"
                  }`}
                >
                  <span>{isActive ? "✓" : "+"}</span>
      ...
      <section className="flex flex-col gap-4 px-4">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e9cece]/30 text-sm font-bold text-slate-900">3</span>
      ...
      <section className="px-4">
        <div className="rounded-xl border border-[#e9cece]/30 bg-white p-6 shadow-sm">
          <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-slate-400">Resumen de tu cita</p>
          <div className="flex flex-wrap gap-6">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-900">{totalDuration} min</p>
            </div>
            <div>
              <p className="text-xs text-slate-400">Precio</p>
              <p className="text-sm font-semibold text-[#e9cece]">₡{selectedService?.price.toLocaleString() ?? "—"}</p>
            </div>
            {date && time && (
              <div>
      ...
      <section className="flex flex-col gap-6 px-4">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e9cece]/30 text-sm font-bold text-slate-900">4</span>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="flex flex-col gap-2 md:col-span-2">
            ...
            <input name="client_name" type="text" placeholder="Jane Doe"
              className="rounded-lg border border-slate-200 bg-white py-3 px-4 text-sm outline-none focus:border-[#e9cece]" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-slate-600">Número de teléfono</label>
            <input name="phone" type="tel" placeholder="+506 6000 0000"
              className="rounded-lg border border-slate-200 bg-white py-3 px-4 text-sm outline-none focus:border-[#e9cece]" />
          </div>
          <div className="flex flex-col gap-2">
